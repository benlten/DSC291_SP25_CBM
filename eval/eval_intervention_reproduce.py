...
def main():
    ...
    for idx in tqdm(range(num_steps)):
        ...
        if idx < 5:
            ...
            if gen_imgs_latent.data[curr_mask].numel() != 0:
                save_image_grid_with_labels(gen_imgs_latent.data[curr_mask], interv_pred[curr_mask], set_of_classes[args.concept_change], grid_size=(4, 4), file_name=save_image_loc+"%d.png" % idx, font_path=font_path)
                save_image_grid_with_labels(gen_imgs_latent_orig.data[curr_mask], recon_pred[curr_mask], set_of_classes[args.concept_change], grid_size=(4, 4), file_name=save_image_loc+"%d_orig.png" % idx, font_path=font_path)
                if args.optint:
                    save_image_grid_with_labels(gen_imgs_dec_latent.data[curr_mask], dec_pred[curr_mask], set_of_classes[args.concept_change], grid_size=(4, 4), file_name=save_image_loc+"%d_dec.png" % idx, font_path=font_path)
            else:
    ...
    savefile_name = f'results/{quant_folder_name}/{args.dataset}_{args.expt_name}_{args.tensorboard_name}.csv'
    if args.optint:
        savefile_name = f'results/{quant_folder_name}/{args.dataset}_{args.expt_name}_{args.tensorboard_name}_optint_eps{args.optint_eps}_iters{args.optint_iters}.csv'
    with open(savefile_name, 'a') as f:
        ...
        f.write(f'{set_of_classes[args.concept_change][args.concept_value]},{num_succ_interv},{num_nottgt_concept}\n')
